...
```bash
# 拉取镜像
docker pull lunare/n8n-runners-debian:2.30.6stable
...
docker run -d \
  ...
  -p 5680:5680 \
  lunare/n8n-runners-debian:2.30.6stable
```
...
```bash
docker pull lunare/n8n-runners-playwright:2.30.6stable
```

**Python 镜像** - 需要 numpy 等 Python 数据处理库时使用
```bash
docker pull lunare/n8n-runners-python:2.30.6stable
```
